...
const workflowContent = {
  paciente: {
    title: "Paciente e Biometria",
    text: "Registre sexo, idade, peso, altura, PA, FC, ritmo e qualidade da janela. Estes dados calibram todos os nomogramas subsequentes."
  },
  plax: {
    title: "Eixo Paraesternal Longo (PLAX)",
    text: "Medir DDVE, DSVE, SIVd, PPd, raiz da aorta, átrio esquerdo e observar dinâmica funcional das valvas mitral/aórtica."
  },
  psax: {
    title: "Eixo Paraesternal Curto (PSAX)",
    text: "Avaliar simetria segmentar, espessura, função global, valva aórtica, mitral e VD conforme varredura de base a ápice."
  },
  a4c: {
    title: "Apical 4 Câmaras (A4C)",
    text: "Avaliar câmaras, FEVE por Simpson, VD, TAPSE, S’ tricúspide, valvas AV e iniciar aquisição de volumes atriais."
  },
  a2c: {
    title: "Apical 2 Câmaras (A2C)",
    text: "Completar Simpson biplanar, calcular volumes finais do VE e AE, analisar motilidade segmentar das paredes anterior e inferior."
  },
  doppler: {
    title: "Hemodinâmica e Doppler",
    text: "Mapeamento espectral: mitral, aórtico, tricúspide, pulmonar, VTI de VSVE, E/e’, TRV e estimativas pressóricas."
  },
  valvas: {
    title: "Integração Valvar",
    text: "Consolidar morfologia, Doppler, gradientes, áreas anatômicas, jatos regurgitantes, impacto cavitário e hipertensão pulmonar associada."
  },
  conclusao: {
    title: "Assistente de Laudo (Pro)",
    text: "O EcoMaster Pro analisa os dados imputados, alerta para inconsistências geométricas ou hemodinâmicas e sugere o corpo de texto estruturado para o laudo."
  }
};
...
function classifyRVFunction(tapse, fac) {
  ...
  if (tapseLow && facLow) return "Reduzida (TAPSE e FAC)";
  if (tapseLow) return "TAPSE reduzido";
  if (facLow) return "FAC reduzida";
  if (Number.isFinite(tapse) && Number.isFinite(fac)) return "Preservada";
  return "--";
}
...
function classifyEF(lvef, sex) {
  ...
  if (lvef >= lower) return "preservada";
  if (lvef >= 41) return "levemente reduzida";
  if (lvef >= 30) return "moderadamente reduzida";
  return "gravemente reduzida";
}
...
function classifyLAVI(lavi) {
  ...
  if (lavi <= 34) return "Volume AE normal";
  if (lavi <= 41) return "Aumento leve";
  if (lavi <= 48) return "Aumento moderado";
  return "Aumento importante";
}

function classifyZ(z) {
  if (!Number.isFinite(z)) return "--";
  if (z < -3) return "Muito abaixo do percentil";
  if (z < -2) return "Abaixo do percentil";
  if (z <= 2) return "Faixa fisiológica";
...
function openDetailDrawer(item) {
  ...
  document.getElementById("drawerDomain").textContent = item.tag || "Referência Clínica";
  ...
  document.getElementById("drawerNote").textContent = item.note || "Interprete as medidas em conjunto com dados de função, clínica do paciente e qualidade técnica da janela ecocardiográfica.";
...
function updateCalculators() {
  ...
  const zSource = document.getElementById("zSource")?.value || "Ref";
  ...
  setText("laviResult", Number.isFinite(lavi) ? `${formatNumber(lavi, 1)} mL/m²` : "--");
  ...
  paintResultState("zResult", classifyZ(z) === "Faixa fisiológica" ? "good" : classifyZ(z) === "--" ? "" : "warn");
...
function generateClinicalSummary() {
  ...
  if (Number.isFinite(r.bsa)) {
    lines.push(`BSA calculada em ${formatNumber(r.bsa, 2)} m².`);
  }

  if (Number.isFinite(r.lvef)) {
    lines.push(`Função sistólica do ventrículo esquerdo globalmente ${r.efClass} (FEVE estimada em ${formatNumber(r.lvef, 0)}%).`);
  }

  if (Number.isFinite(r.lvmi) && Number.isFinite(r.rwt)) {
    lines.push(`Geometria ventricular: ${r.geometry} (Massa indexada: ${formatNumber(r.lvmi, 0)} g/m²; ERP: ${formatNumber(r.rwt, 2)}).`);
  }

  if (Number.isFinite(r.lavi)) {
    lines.push(`${r.laviClass} (Volume indexado: ${formatNumber(r.lavi, 1)} mL/m²).`);
  }

  if (Number.isFinite(r.psap)) {
    lines.push(`PSAP estimada em ${formatNumber(r.psap, 0)} mmHg. Sugere-se correlacionar com o estado clínico.`);
  }

  if (r.rvFunction !== "--") {
    lines.push(`Função sistólica do ventrículo direito: ${r.rvFunction}.`);
  }
  ...
  if (lines.length === 0) {
    lines.push("Preencha as calculadoras clínicas acima para que a inteligência do EcoMaster gere uma proposta de laudo estruturada.");
  }
  ...
  showToast("Síntese técnica gerada com sucesso.");
}
...
function initCalculatorListeners() {
  const ids = [
    "sexInput", "ageInput", "weightInput", "heightInput",
    "trvInput", "rapInput", "tapseInput", "facInput",
    "lviddInput", "ivsdInput", "pwdInput", "lvefInput",
    "lavInput", "zInput", "zSource"
  ];
...
function initWorkflow() {
  ...
  document.querySelectorAll(".workflow-tab").forEach(button => {
    button.addEventListener("click", () => {
      ...
      if (type === "laudo") {
        setText("workflowTitle", "Protocolos Específicos");
        setText("workflowText", "Integre protocolos de Cardiotoxicidade, TAVI, MitraClip ou Viabilidade miocárdica guiada por estresse.");
      } else {
        setText("workflowTitle", "Medidas Iniciais");
        setText("workflowText", "Certifique-se de registrar PA, FC, ritmo cardíaco, peso e altura para indexação correta antes de iniciar as capturas.");
      }
    });
  });
}
...
function openLeadModal(source = "geral") {
  ...
  } else if (source === "ve" || source === "psap" || source === "laudo") {
    title.textContent = "Assistente de Laudo Pro";
    text.textContent = "Gere frases estruturadas, clinicamente validadas e prontas para copiar e colar no prontuário. Registre-se para experimentar o fluxo guiado do EcoMaster Pro.";
  } else if (source === "zscore") {
    title.textContent = "Z-Score Pediátrico Multiparamétrico";
    text.textContent = "Cálculo instantâneo baseado nas diretrizes de Boston, Detroit ou CHOP para todas as estruturas fetais e pediátricas. Função exclusiva da versão Pro.";
  } else {
    title.textContent = "Acesso EcoMaster Pro";
    text.textContent = "Cadastre-se para ser notificado assim que as funções de laudo assistido e calculadoras avançadas estiverem liberadas.";
  }
...
function submitLead(event) {
  ...
  if (!name || !profile || !email) {
    showToast("Por favor, preencha todos os dados.");
    return;
  }

  closeLeadModal();
  showToast(`Acesso antecipado registrado, Dr(a). ${name}. Em breve entraremos em contato.`);
  event.target.reset();
}
